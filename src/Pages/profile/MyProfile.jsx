  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (name) {
          const response = await axios.get(`https://careerdrishti-fsm-backend.onrender.com/api/users?name=${name}`);
          setUserId(response.data.userId);
        }

        // Fetch quiz results based on userId
        if (userId) {
          const resultsResponse = await axios.get(`https://careerdrishti-fsm-backend.onrender.com/api/quiz-results/${userId}`);
